...
const bootstrapModulePath = process.env.OPENAPI_BOOTSTRAP_MODULE || '../build/src/app.js'
const { bootstrap } = require(bootstrapModulePath)
...
function orderOpenApiDocument(openapi) {
  const ordered = orderKeys(openapi, [
    'openapi',
    'info',
    'servers',
    'tags',
  ...
  for (const key of Object.keys(openapi)) {
    delete openapi[key]
  }
  Object.assign(openapi, ordered)
}

function orderKeys(value, preferredOrder) {
  const ordered = {}
