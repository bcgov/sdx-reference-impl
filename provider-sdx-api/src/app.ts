...
const apiDescription = `SDX-facing provider API for managing fictional widgets.

This API derives widget ownership from the authenticated subject in the JWT \`sub\` claim. Callers cannot set or change the subject through paths or request bodies.

Requests are adapted and proxied to the non-SDX-facing provider API, which receives the owner subject through path or body parameters.

The Widget scopes are \`nrs:widgets:read\`, \`nrs:widgets:create\`, \`nrs:widgets:update\`, and \`nrs:widgets:delete\`.`

const DEFAULT_OIDC_AUTHORITY = 'https://identity.example.com/realms/sdx'
const DEFAULT_SWAGGER_CLIENT_ID = 'local-provider-sdx-api-swagger'
...
function alignGeneratedWidgetSpec(document: OpenAPIObject, apiServerPath: string) {
  ...
  const orderedPaths = ['/widgets', '/widgets/{widgetId}']
