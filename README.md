...
| `PROVIDER_API_CLIENT_ID` | Yes | Generated by setup script | Confidential service client ID used by `provider-sdx-api` |
| `PROVIDER_API_CLIENT_SECRET` | Yes | | Confidential service client secret used for client-credentials token requests |
| `PROVIDER_API_TOKEN_SCOPE` | No | | Optional scopes requested on the client-credentials token |
| `PROVIDER_API_TOKEN_URL` | No | OIDC discovery `token_endpoint` | Explicit token endpoint for service-token requests |
